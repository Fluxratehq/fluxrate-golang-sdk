...
func main() {
	...
	sdk, err := billing.NewSDK(billing.Config{
		...
		BatchInterval:    5 * time.Second,                                              // Flush batch every 5 seconds
		AllowedCustomers: []string{"user_123", "user_456", "user_789", "user_premium"}, // Only track these customers
	})
	if err != nil {
...
func simulateAPIUsage(ctx context.Context, sdk *billing.SDK, meterToken string) error {
	...
	// Example 6: Customer filtering (disallowed customer)
	fmt.Println("\n📊 Example 6: Disallowed customer (filtered out)")
	fmt.Println("   Note: SDK is configured to only track specific customers")
	result, err = sdk.Track(ctx, billing.TrackEventParams{
		MeterToken:         meterToken,
		CustomerExternalID: "user_not_allowed", // This customer is NOT in AllowedCustomers list
		Quantity:           1,
	})
	if err != nil {
		fmt.Printf("❌ Error: %v\n", err)
	} else if result == nil {
		fmt.Println("✅ Event skipped (customer not in allowed list)")
		fmt.Println("   Check debug logs above for: 'Customer not in allowed list'")
	} else {
		fmt.Printf("✅ Event tracked: %s\n", result.ID)
	}
